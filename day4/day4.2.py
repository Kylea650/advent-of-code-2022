...
def fully_contains(pairs: list[list[list[str]]]) -> int:
    ...
    there is an intersection and updates the list to True for that index

    Returns: the count of instances of True
    """

    intersects_list = [False] * len(pairs)

    for i in range(len(pairs)):
        if int(pairs[i][0][1]) >= int(pairs[i][1][0]) and int(pairs[i][1][1]) >= int(
            pairs[i][0][0]
        ):
            intersects_list[i] = True

    true_count = intersects_list.count(True)
